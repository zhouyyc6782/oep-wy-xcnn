...
def main():
    mol = gto.M(atom=sys.argv[1], basis=sys.argv[3], charge=int(sys.argv[2]), spin=0)
    mr = scf.RKS(mol)
    mr.xc = "B3LYPG"
    mr.grids.build()
    # mr.run()
    ...
    # dm_cc, _ = ccsd(mol)
    dms = [numpy.load(fn) for fn in sys.argv[4:]]
    # dms.insert(0, dm_cc)
    # dms.insert(1, dm_rks)
    labels = sys.argv[4:]
    # labels.insert(0, "cc")
    ...
    dms = numpy.array(dms)
    print("[34m======== I ========[0m")
    for i, dm in enumerate(dms[0:]):
        numerator, denominator = compare_density(
                mol, dm, dms[0], mr.grids, "I", 
                full_output=True)
        print("%20s\t%16.8e / %16.8e = %16.8e" % (labels[i], numerator, denominator, numerator / denominator))
